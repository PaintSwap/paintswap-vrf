...
import {PaintswapVRFCoordinatorCore} from "../PaintswapVRFCoordinatorCore.sol";
 ...
 */
contract MockVRFCoordinator is PaintswapVRFCoordinatorCore {
  /* -------------------------------------------------------------------------- */
  /*                                Constants                                   */
  /* -------------------------------------------------------------------------- */
   ...
   * @param refundee Address to refund any unused gas
   */
  function fulfillRequestMock(
    ...
    uint256 gasUsed = gasLeft - gasleft(); // Calculate gas used
    _refundRemainingGas(
      requestId,
      request.callbackGasLimit,
      gasUsed,
      request.gasPricePaid,
      refundee
    );
  }
  ...
  function fulfillRequestMockWithRandomWords(uint256 requestId) external {
    ...
    uint256 gasLeftPreFullfillment = gasleft();
    _fulfillRequest(requestId, randomWords);
    _refundRemainingGas(
      requestId,
      request.callbackGasLimit,
      gasLeftPreFullfillment - gasleft(),
      request.gasPricePaid,
      request.refundee
    );
  }
  ...
  function requestRandomnessPayInNative(
    ...
    uint256 commitment = _getCommitmentHash(
      requestId,
      consumer,
      callbackGasLimit,
      numWords,
  ...
  function fulfillRandomWords(
    ...
    uint256 gasLeftPreFullfillment = gasleft();
    bool result = _fulfillRequest(requestId, randomWords);
    _refundRemainingGas(
      requestId,
      callbackGasLimit,
      gasLeftPreFullfillment - gasleft(),
  ...
  function _fulfillRequest(
    ...
    if (success) {
      ...
      emit ConsumerCallbackFailed(requestId, 3, request.consumer, gasleft());
          ...
          if (returnData.length > 4) {
            ...
            try this.decodeErrorString(errorData) returns (
              ...
              if (bytes(errorMessage).length > 0) {
                reason = string.concat("callback failed: ", errorMessage);
              } else {
  ...
  function getRequestResult(
    ...
    return (_requestResults[requestId] == CallbackStatus.SUCCESS, fulfilled);
  }
