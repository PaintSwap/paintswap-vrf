...
abstract contract PaintswapVRFConsumer is IPaintswapVRFConsumer {
  ...
  function _requestRandomnessPayInNative(
    ...
    return _vrfCoordinator.requestRandomnessPayInNative{value: value}(callbackGasLimit, numWords);
  }
   ...
   */
  function _calculateRequestPriceNative(uint256 callbackGasLimit) internal view returns (uint256 requestPrice) {
    requestPrice = _vrfCoordinator.calculateRequestPriceNative(callbackGasLimit);
  }
   ...
   */
  function rawFulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) external override onlyCoordinator {
